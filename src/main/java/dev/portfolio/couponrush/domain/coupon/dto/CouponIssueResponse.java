...
public class CouponIssueResponse {
    ...
    // 쿠폰 발급 내역의 식별자를 반환함
    @Schema(description = "쿠폰 발급 내역 식별자", example = "1")
    private final Long id;

    // 발급된 쿠폰의 식별자를 반환함
    @Schema(description = "발급된 쿠폰 식별자", example = "1")
    private final Long couponId;

    // 쿠폰을 발급받은 사용자의 식별자를 반환함
    @Schema(description = "쿠폰을 발급받은 사용자 식별자", example = "1")
    private final Long userId;

    // 쿠폰 발급 내역의 현재 상태를 반환함
    @Schema(description = "쿠폰 발급 상태", example = "ISSUED")
    private final CouponIssueStatus status;

    // 쿠폰이 발급된 일시를 반환함
    @Schema(description = "쿠폰 발급 일시", example = "2026-09-04T10:30:00")
    private final LocalDateTime issuedAt;

    // 쿠폰이 사용된 일시를 반환함
    @Schema(
            description = "쿠폰 사용 일시이며 사용 전에는 null임",
            nullable = true
    )
    ...
    @Schema(
            description = "쿠폰을 사용한 주문 식별자이며 사용 전에는 null임",
            nullable = true
    )
